...
class APIController extends Controller
{
    ...
    public function listItinerariesByCountry(Request $request)
    {
      ...
      $itineraries->getCollection()->transform(function ($itinerary){
        ...
        $totalcomments = json_decode($this->getTotalComments($newReq),true);
        $isLiked = $this->isLiked(Auth::user()->id, $itinerary->id);
        $user_rank = $this->getUserRank(Itinerary::where('user_id', $itinerary->user->id)->count());
    ...
    public function listTrendingItinerariesByCountry(Request $request)
    {
      $country_id = $request->country_id;
      $numdata = 5; // set total contents to display per page
      $itineraries = Itinerary::with(['country','user'])
    ...
    // (SORT TRENDING - MOST COMMENTS)
    public function listTrendingItinerariesByUserPaginated(Request $request)
    {
      $user_id = $request->user_id;
      $numdata = 5; // set total contents to display per page
      $itineraries = Itinerary::with(['country','user'])
    ...
    // (SORT TOP - MOST LIKES)
    public function searchTopItineraries(Request $request)
    {
      $searchTitle = $request->title;
      $numdata = 5; // set total contents to display per page
      $itineraries = Itinerary::with(['country','user'])
    ...
    public function getUserData(Request $request)
    {
        ...
        $user->rank = $this->getUserRank($totalitineraries);

        return $user;
    }

    // Set get rank, return badge
    public function getUserRank($total)
    {
      $badge = [];
      if($total >= 0 && $total <= 5) {
